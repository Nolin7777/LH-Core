        ...
        if (!target->getTransForm() || !IsPositiveSpell(GetId()) || IsPositiveSpell(target->getTransForm()))
        {
            ...
            else
            {
                ...
                if (ci && target->GetTypeId() == TYPEID_UNIT)
                    ((Creature*)target)->LoadEquipment(ci->EquipmentTemplateId, true);
            }
    ...
    else
    {
        ...
        if(target->GetTypeId() != TYPEID_PLAYER)
        {
            ...
            if (((Creature*)target)->IsPet())
            {
                ...
                target->setFaction(cinfo->FactionHorde);

            // restore UNIT_FIELD_BYTES_0
            if (cinfo && caster && caster->GetTypeId() == TYPEID_PLAYER && caster->getClass() == CLASS_WARLOCK && cinfo->CreatureType == CREATURE_TYPE_DEMON)
            {
