        ...
        if (!AddListe.empty())
        {
            for (std::list<Creature*>::iterator itr = AddListe.begin(); itr != AddListe.end(); ++itr)
            {
                if ((*itr)->isAlive() && AddVivant > 0)
                {
                    ...
                    (*itr)->SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, cinfo->MinMeleeDmg + cinfo->MinMeleeDmg / AddVivant);
                    (*itr)->SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, cinfo->MaxMeleeDmg + cinfo->MaxMeleeDmg / AddVivant);
                }
            }
        }
