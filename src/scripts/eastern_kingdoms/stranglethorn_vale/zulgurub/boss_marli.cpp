    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (m_uiTransformBack_Timer < uiDiff)
        {
            if (!m_bIsInPhaseTwo)
            {
                ...
                m_creature->SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, (cinfo->MinMeleeDmg + ((cinfo->MinMeleeDmg / 100) * 35)));
                m_creature->SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, (cinfo->MaxMeleeDmg + ((cinfo->MaxMeleeDmg / 100) * 35)));
            ...
            else
            {
                ...
                const CreatureInfo *cinfo = m_creature->GetCreatureInfo();
                m_creature->SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, (cinfo->MinMeleeDmg + ((cinfo->MinMeleeDmg / 100) * 1)));
                m_creature->SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, (cinfo->MaxMeleeDmg + ((cinfo->MaxMeleeDmg / 100) * 1)));
                m_creature->UpdateDamagePhysical(BASE_ATTACK);
